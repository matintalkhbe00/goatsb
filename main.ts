...
async function handleToken(authToken: string, phoneNumber: string) {
  ...
  while (true) {
    ...
    if (now >= nextTime) {
      ...
      if (result) {
        totalReward += 200; // اضافه کردن ریوارد به مقدار کل
        console.log(`Success: Action to earn was successfully completed with phone number ${phoneNumber}`);
        console.log(`Total Reward: ${totalReward}`); // چاپ مقدار کل ریوارد
...
async function makeMoney(authTokensAndPhones: { token: string; phone: string }[]) {
